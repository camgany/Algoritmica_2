...
int maxFlow(int s,int t) { 
    ...
    while(true) {// empezamos con true porque nosotros nunca sabemos si tenemos flujo
        ...
        while(!Q.empty() && path[t]==-1) {//= 2
            int currentNode = Q.front();//saco la s//= 1
            Q.pop();//elimino de la cola//= 1
            for(int i = 0 ; i < MAX_N ; i++) {//buscamos a sus amigos//= peor caso, que recorra todos las aristas
